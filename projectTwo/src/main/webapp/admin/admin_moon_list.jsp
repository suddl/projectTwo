...
<a href="<%=request.getContextPath()%>/index.jsp?group=admin&worker=admin_moon_list"><h1>1:1 문의하기</h1></a>
<div id="moon_list">
	...
	<table class="moon_list">
		...
		<% } else { %>
			...
			<tr>
				...
				<td class="subject">
					<%
						String url=request.getContextPath()+"/index.jsp?group=admin&worker=admin_moon_detail&moonNum="+moon.getMoonNum()
	...
	<div id="page_list">
		<%
			String responseUrl=request.getContextPath()+"/index.jsp?group=admin&worker=admin_moon_list"
	...
	<form action="<%=request.getContextPath() %>/index.jsp?group=admin&worker=admin_moon_list" method="post">
		<select name="search">
...
$("#writeBtn").click(function() {
	location.href="<%=request.getContextPath()%>/index.jsp?group=admin&worker=admin_moon_write";	
});
</script>