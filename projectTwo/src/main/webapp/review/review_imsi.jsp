...
<%
    request.setAttribute("reviewSubject", "리뷰 제목 예시");
    request.setAttribute("reviewContent", "리뷰 내용 예시");
    request.setAttribute("reviewImage", "리뷰 내용 예시");
    // 기타 필요한 데이터 설정
%>
<%
...
#submitBtn {
        ...
        margin-left: 150px; /* 버튼을 오른쪽으로 20px 이동 */
    }
#reviewContent {
	height: 500px;
	width: 350px;
...
<form action="<%=request.getContextPath()%>/index.jsp?group=review&worker=review_writer_action"
    ...
    <div class="star-ratings-fill" style="width: 0%;">★★★★★</div>
    <div class="star-ratings-base">★★★★★</div>&nbsp;&nbsp;
</div>&nbsp;&nbsp;
 
 <script type="text/javascript">
    // 별점 클릭 이벤트 핸들러 설정
    document.addEventListener('DOMContentLoaded', function () {
        const stars = document.querySelectorAll('.star-ratings-base > span');
    ...
    // 별점 설정 함수
    function setRating(ratingPercent) {
        const fill = document.querySelector('.star-ratings-fill');
		...
		<tr>
			<th colspan="2">
				<button type="submit" id="submitBtn">글저장</button>
			</th>
		</tr>
	</table>
</form>
